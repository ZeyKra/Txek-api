...
import { deleteRound, getRoundInformations, updateRoundInformations } from "@/app/backend/surreal-actions";
...
export async function PATCH(request: Request, { params }: { params: { matchId: string; roundId: string } }) {
  try {
    ...
    // TODO: Check if match exist
    const updateInformations = await updateRoundInformations(`Round:${roundId}`, updateData);
